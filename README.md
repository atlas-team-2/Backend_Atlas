...
3. Заполните `.env` — обязательно укажите:
- `AUTH__SECRET` — сгенерируйте командой `openssl rand -hex 32`
- `RBAC__ADMIN_EMAIL` и `RBAC__ADMIN_PASSWORD` — данные для первого администратора
...
5. API будет доступно по адресу: `http://localhost/api/v1`
